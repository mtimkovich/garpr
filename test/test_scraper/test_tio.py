...
class TestTioScraper(unittest.TestCase):
    ...
    def test_get_matches(self):
        ...
        self.assertEquals(matches[0], {'winner':'spookyman', 'loser':'razr'})

        # grand finals set 2
        self.assertEquals(matches[-1], {'winner':'GC|silent wolf', 'loser':'MIOM|SFAT'})

        # grand finals set 1
        self.assertEquals(matches[-2], {'winner':'MIOM|SFAT', 'loser':'GC|silent wolf'})

        # losers finals
        self.assertEquals(matches[-3], {'winner':'MIOM|SFAT', 'loser':'shroomed'})
    ...
    def test_get_matches_one_grand_finals_set(self):
        ...
        # losers finals
        self.assertEquals(matches[-2],{'winner':'MIOM|SFAT', 'loser':'shroomed'})
