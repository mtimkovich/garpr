...
class TestTioScraper(unittest.TestCase):
    ...
    def test_get_matches(self):
        ...
        self.assertEquals(matches[0],
                          AliasMatch(winner='spookyman', loser='razr'))

        # grand finals set 2
        self.assertEquals(matches[-1],
                          AliasMatch(winner='GC|silent wolf',
                          loser='MIOM|SFAT'))

        # grand finals set 1
        self.assertEquals(matches[-2],
                          AliasMatch(winner='MIOM|SFAT',
                          loser='GC|silent wolf'))

        # losers finals
        self.assertEquals(matches[-3],
                          AliasMatch(winner='MIOM|SFAT', loser='shroomed'))

    def test_get_matches_invalid_bracket_name(self):
        with self.assertRaises(ValueError):
            self.scraper = TioScraper.from_file(FILEPATH,
                                                'invalid bracket name')
            self.scraper.get_matches()

    def test_get_matches_one_grand_finals_set(self):
        self.scraper = TioScraper.from_file('test/test_scraper/data/2.tio',
                                            BRACKET_NAME)
        matches = self.scraper.get_matches()
